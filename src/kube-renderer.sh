    ...
    if [[ "version" == "$1" ]]; then
        ...
        if [[ -n "${HELMOUTPUTDIR}" && "${HELMOUTPUTDIR}" =~ ^.*helmx\.[[:digit:]]+\.rendered$ ]]; then
            ...
            for FILE in $(find "${HELMOUTPUTDIR}/" -type f | sort | sed "s|^${HELMOUTPUTDIR}/||"); do
                # Prevent source comment duplicity
...
EOF
    ...
    # shellcheck disable=SC2016
    yq eval -N '.releases // ""' "${TMPDIR}/helmfile-values/globals.yaml" | yq eval '.[] |= .values = (.values[] as $item ireduce ({}; . * $item )) | .[].values |= .".kube-renderer"=.".kube-renderer" // {}' > "${TMPDIR}/helmfile-values/releases.yaml"
...
if [[ -z "${LOCAL_HELM_CACHE}" ]]; then
    ...
    HELM_CACHE_HOME="${TMPDIR}/helmhome"
    HELM_CONFIG_HOME="${TMPDIR}/helmhome"
    HELM_DATA_HOME="${TMPDIR}/helmhome"
